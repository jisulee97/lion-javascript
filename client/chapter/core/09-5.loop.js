...
// lierable(반복 가능한 요소들) = String, Array, ArrayLike(유사배열)
// 인덱스와 length 를 가지고 있으면 배열이지만 아닌 경우도 있다
...
for (let value of languages) {
  ...
  // includes
  // 사용 시 Java 라는 글씨에서부터 걸려 출력이 안 되기 때문에 길이 조건을 추가로 걸어줌
...
for (let keyValue of Object.entries(randomUser)) {
  ...
  console.log('/t/tL1 : ', value);

  if (typeof value === 'object') {
    let key = keyValue[0];
    let value = keyValue[1];
    console.log('/t/tL2 : ', value);

    if (typeof value === 'object') {
      for (let keyValue of Object.entries(value)) {
        let key = keyValue[0];
        let value = keyValue[1];
        console.log('/t/tL3 : ', value);
      }
    }
  }
}
